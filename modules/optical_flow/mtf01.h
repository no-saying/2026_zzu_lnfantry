#pragma once

#include <stdint.h>
#include <stdbool.h>
#include <string.h>
#include "bsp_usart.h"

#define MICOLINK_MSG_HEAD            0xEF
#define MICOLINK_MAX_PAYLOAD_LEN     64
#define MICOLINK_MAX_LEN             MICOLINK_MAX_PAYLOAD_LEN + 7

/*
    消息ID定义
*/
enum
{
    MICOLINK_MSG_ID_RANGE_SENSOR = 0x51,     // 测距传感器
};

/*
    数据负载定义
*/
#pragma pack (1)
// 测距传感器数据结构
typedef struct
{
    uint32_t  time_ms;               // 系统时间 ms
    uint32_t  distance;              // 距离(mm) 最小值为10，0表示数据不可用
    uint8_t   strength;              // 信号强度
    uint8_t   precision;             // 精度
    uint8_t   tof_status;            // 状态
    uint8_t   reserved1;             // 预留
    int16_t   flow_vel_x;            // 光流速度x轴
    int16_t   flow_vel_y;            // 光流速度y轴
    uint8_t   flow_quality;          // 光流质量
    uint8_t   flow_status;           // 光流状态
    uint16_t  reserved2;             // 预留
} MICOLINK_PAYLOAD_RANGE_SENSOR_t;
#pragma pack ()

/*
    消息结构体定义
*/
typedef struct
{
    uint8_t head;                      
    uint8_t dev_id;                          
    uint8_t sys_id;						
    uint8_t msg_id;                        
    uint8_t seq;                          
    uint8_t len;                               
    uint8_t payload[MICOLINK_MAX_PAYLOAD_LEN]; 
    uint8_t checksum;                          

    uint8_t status;                           
    uint8_t payload_cnt;                       
} MICOLINK_MSG_t;

// 对外提供的传感器数据结构（根据你的master_process.h风格）
#pragma pack(1)
typedef struct
{
    uint32_t timestamp;      // 时间戳
    uint32_t distance;       // 距离(mm)
    uint8_t strength;        // 信号强度
    uint8_t precision;       // 精度
    uint8_t tof_status;      // TOF状态
    int16_t flow_vel_x;      // 光流速度X (cm/s@1m)
    int16_t flow_vel_y;      // 光流速度Y (cm/s@1m)
    uint8_t flow_quality;    // 光流质量
    uint8_t flow_status;     // 光流状态
    bool is_valid;           // 数据是否有效
} MTF01_Recv_s;
#pragma pack()

// 公共函数声明
MTF01_Recv_s *MTF01Init(UART_HandleTypeDef *_handle, void (*application_callback)(void));
void MTF01Decode(uint8_t *data, uint16_t size);